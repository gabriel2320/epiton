...
| Shell hosts | Tauri / Capacitor | Beta | Title/safe-area; memory-only sessions; legacy preference slots erased; CI APK/Tauri producers define SHA-256/source/toolchain receipts and push attestations. A fail-closed promotion verifier now requires exact signed candidates, clean `main`, current toolchains, separated authenticated signing/device authorities and every physical-device scenario; first green remote candidates and those external approvals remain pending |
