...
| Model CRUD | `model.*.create/read/write/delete/search_read` | Implemented | Generic `ModelWorkspace`; reads request Tryton's `_timestamp` and mutations return the oldest parent/nested snapshot only in the reserved RPC context, so stale writes are rejected without leaking metadata into business values or x2many tuples |
...
| O2M / M2M / M2O | field types in view engine | Improved | Shared web relation hosts consume the frozen child Screen contract for create/write/remove/cancel/validation, latest-response guards, x2many replacement/patch translation, and metadata-driven server `pre_validate`; dirty-exit decisions bubble through nested editors, a rejected switch preserves the same draft, and parent Save cannot bypass an open line. M2M preserves nested commands plus membership delta. Deterministic O2M and M2M receipts each prove one parent write with no child mutation; the M2M wire is exactly add `22` / remove `20`. A Tryton 7 live metadata receipt qualifies transient child `on_change_with` + `pre_validate`; broader third-party/series depth is not claimed |
| Revision history | `model.*.history_revisions(ids)` + `model.*.read(ids, fields, {_datetime})` | Improved | Native revision metadata and read-only temporal snapshots; restore enters the ordinary writable draft/ACL path |
| Notebook | form notebook pages | Improved | Exclusive tabs + process-local current page |
| Buttons | view `button` + confirm | Improved | Method RPC with canonical `active_id(s)` context; `type=action` → shared resolveAction host; typed toolbar policy is shared by list/tree/record paths; model-button hosts synchronously admit only one in-flight RPC and expose disabled/busy state until it settles |
...
| Reports | `report.*.execute(ids, data, context)` | Improved | Native Tryton parameter shape, encoded bytes envelope, backend-owned extension/filename, MIME-aware download, pdfjs preview and `ir.action.report` picker; board→shared Shell mock proof preserves action context, while the live GNU Health gate previews `patient.card` as a real PDF |
...
| Shell hosts | Tauri / Capacitor | Beta | Title/safe-area; memory-only sessions; legacy preference slots erased; ordinary CI APK/Tauri producers define SHA-256/source/toolchain receipts and push attestations. A manual protected workflow now signs/verifies Android and emits exact Linux release candidates from clean `main`; its receipts remain non-promotable. The fail-closed promotion verifier requires current toolchains, independently authenticated signing/device authorities and every physical-device scenario; first green remote candidates and those external approvals remain pending |
...
| Screen host | act_window lifecycle + relation queue | Improved | Explicit parent/child hydrate lifecycle; pristine `default_get`; A→B/generation + last-request-wins guards; Save/Accept flush pending `on_change`; parent save via `screenValuesForSave`; open child drafts disable parent Save/Ctrl+S and every replacing transition confirms before discard. Deterministic browser proof covers preserved rejected switches, O2M queued create+edit, and M2M membership add/remove as one parent write with no child mutation. Tryton 7 live metadata proves the transient child `on_change_with` / `pre_validate` boundary; late-A isolation from B writes remains covered. Nested lifecycle: [`CHILD_SCREEN_CONTRACT.md`](CHILD_SCREEN_CONTRACT.md) |
...
| Next.js App Router host | Same Epitón application and contracts | N1 partial | Build and 15-scenario production E2E/CSP/PWA receipt pass; ordinary and protected native receipt/attestation producers are defined, while their first green CI evidence and external approvals remain before the N2 web cutover |
...
| GNU Health | `gnuhealth.*` models | Core `health` clinical slice verified | The pinned Tryton 8/PostgreSQL Chilean core proves Spanish context, the exact eight-module activation set and five critical view contracts. A live gateway/browser gate performs person/patient CRUD, appointment create → `checked_in` → delete, evaluation create/update with delete denied, prescription with nested line create → finalize, vaccination create → sign, and translated patient-card action → backend-owned PDF preview. It holds prescription finalization in flight and proves that disabled/busy state plus synchronous admission prevents a duplicate RPC; Tryton then rejects terminal button replay and the backend retains one longitudinal event per protected document. Two independent Spanish Epitón sessions also prove native `_timestamp` locking: the first write refreshes the snapshot, Tryton rejects the stale second write, and the newest clinical value remains authoritative. The patient history panel then obtains native revision metadata and reads both the current and immediately previous clinical snapshot through `_datetime`, without treating the history table as an RPC model. The backend verifies protected-state immutability, longitudinal events and zero residual fixture records, proves a live backup/restore, and checks Tryton's effective ACLs for eight identities on five protected models with negative cases and no persistent demo users. All eight profiles also pass translated menu, model-access and create-control browser journeys in a disposable database excluded from the operational backup. Temporal model history remains separate from the authoritative RPC audit: a shared Epitón/Tryton correlation UUID links successful central disclosures, mutations and buttons to 68 minimal SHA-256-linked events, and the exact chain-head receipt survives backup, restore and cleanup. Native JSON-RPC has no durable idempotency-key guarantee, and PHI, migration, clinical approval and production compliance remain separate gates |
