...
| **shadcn/ui** | Component recipes | **Adopt selectively** | Already use Radix + CVA + `tailwind-merge` + `clsx`. Prefer porting recipes into `@epiton/ui` over wiring the shadcn CLI / dumping a `components/ui` tree. |
