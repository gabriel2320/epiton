...
- No claim is made for penetration testing, WCAG conformance, PHI readiness,
  clinical certification, native device acceptance or release signing without
  an exact governed promotion receipt, edge DDoS capacity, IdP integration, or
  secret rotation.
...
| T-10 | Compromised dependency, build, or service worker | Pinned lockfiles, production JavaScript advisory audit, RustSec, unit/build gates, and bundle budget are defined. Every CI native bundle gets a SHA-256 receipt bound to the source revision, working-tree state, CI run, declared toolchains, and lockfile digest; push builds also receive GitHub artifact attestations. The native promotion verifier additionally requires a clean exact revision, current toolchain, repository-scoped attestation and separately authenticated external approvals. Approval evidence carries Ed25519 signatures checked against distinct policy-pinned public-key fingerprints; the verifier never receives private approval keys. The PWA precaches versioned static assets only and has no RPC/auth runtime cache. | An attestation or valid approval signature does not make a compromised workflow or authority trustworthy. Protect CI and both authorities' private keys, review trust-root and lockfile changes, retain raw verification evidence, and rotate/revoke through institutional procedure. |
...
| T-12 | Native WebView persistence or artifact substitution | Desktop/mobile sessions remain in memory; legacy cleanup APIs remove exact historical entries without hydration. CI verifies the Android debug signature, emits native receipts/checksums, and attests push-built Android APK and Linux Tauri subjects. Android keystores are explicitly ignored. The fail-closed native promotion contract rejects debug/unsigned bytes, dirty or wrong revisions, stale/tampered evidence, unconfigured/reused trust roots, virtual-device acceptance and incomplete lifecycle scenarios. | Current CI outputs remain debug/unsigned, approval trust roots remain deliberately unconfigured, and no production promotion receipt exists yet. Signing and device authorities must verify exact release bytes externally, protect separate private keys, retain raw evidence, govern the public trust roots, and issue the first accepted remote receipt before distribution. |
