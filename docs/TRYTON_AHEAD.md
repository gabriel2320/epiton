...
2. Richer report/pdfjs pipeline & email compose wizards
3. Lazy `field_childs` fetch / `tree_state` persistence / DnD sequence

Recently improved (2026-07-31): hierarchical tree expand, server menu favorites,
board `_actions` + multi-y, form-in-pane, `ir.ui.view_search`, translation catalog,
attachment DnD, CSV map, revision peek, editable tree, notebook, wizard, bus.
...
| **Hierarchical tree** | Parent expand via TreeMixin / `field_childs` | **Improved:** client flatten + expand column when `parent` Many2One | Small–Medium | `tree_hierarchy.ts` |
...
3. ~~Hierarchical tree expand~~; ~~server menu favorites~~
4. Lazy `field_childs` fetch + `ir.ui.view_tree_state`; report/pdfjs; email compose
